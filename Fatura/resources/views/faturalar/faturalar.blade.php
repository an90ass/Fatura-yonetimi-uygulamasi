...
@if (session()->has('delete_fatura'))
    ...
    @endif
    @if (session()->has('restore_fatura'))
    <script>
        window.onload = function() {
            notif({
                msg: "Fatura başarıyla geri yüklendi",
                type: "success"
            })
        }
    </script>
					...
					<div class="col-xl-12">
						<div class="card mg-b-20">
							<div class="card-header pb-0">
								...
								<a href="faturalars/create" class="modal-effect btn btn-sm btn-primary" style="color:white"><i
                                                                ...
                                                                <a class="dropdown-item"
                                                                    ...
                                                                    <a class="dropdown-item" href="#" data-fatura_id="{{ $x->id }}"
                                                                        ...
                                                                        href="{{ url('Durumu_goster') }}/{{ $x->id }}"><i
                                                                            ...
                                                                            <a class="dropdown-item" href="#" data-fatura_id="{{ $x->id }}"
                                                                                data-toggle="modal" data-target="#arsive_gonderme"><i
                                                                                class="text-warning fas fa-exchange-alt"></i>&nbsp;&nbsp;
                                                                                    Arşive gönder</a>

                                                                                    <a class="dropdown-item" href="print_fatura/{{ $x->id }}"><i
...
<!-- fatura arsivi  -->
<div class="modal fade" id="arsive_gonderme" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
aria-hidden="true">
<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Faturayı arşivleme </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            <form action="{{ url('faturalars/destroy') }}" method="post">
        ...
        <div class="modal-body">
            ?Arşivleme sürecinden emin misiniz
            <input type="hidden" name="fatura_id" id="fatura_id" value="">
...
<script>
    $('#arsive_gonderme').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget)
        var fatura_id = button.data('fatura_id')
        var modal = $(this)
        modal.find('.modal-body #fatura_id').val(fatura_id);
