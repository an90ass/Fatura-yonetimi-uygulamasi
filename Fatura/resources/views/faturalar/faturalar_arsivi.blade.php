...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">Faturalar</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ faturaların arşivi </span>
						</div>
					</div>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu tx-13">
                                                            ...
                                                            <a class="dropdown-item" href="#" data-fatura_id="{{ $x->id }}"
                                                                data-toggle="modal" data-target="#fatura_donusturme"><i
                                                                class="text-warning fas fa-exchange-alt"></i>&nbsp;&nbsp;
                                                                    Fatura listesine gönderme</a>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <form action="{{ url('Arsiv/destroy') }}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
            </div>
...
<!-- fatura listesine gonderme  -->
<div class="modal fade" id="fatura_donusturme" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
aria-hidden="true">
<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Fatura arşivlemeyi iptal etme </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            <form action="{{ url('Arsiv/update') }}" method="post">
                {{ method_field('patch') }}
                {{ csrf_field() }}
        </div>
        <div class="modal-body">
            ?Arşivleme işlemini iptal etmek istediğinizden emin misiniz
...
<script src="{{URL::asset('assets/js/table-data.js')}}"></script>
 ...
 <script>
    $('#delete_fatura').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget)
        var fatura_id = button.data('fatura_id')
        var modal = $(this)
        modal.find('.modal-body #fatura_id').val(fatura_id);
    })
</script>
<script>
    $('#fatura_donusturme').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget)
        var fatura_id = button.data('fatura_id')
        var modal = $(this)
        modal.find('.modal-body #fatura_id').val(fatura_id);
